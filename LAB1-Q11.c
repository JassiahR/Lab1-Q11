...
{  
    ...
    while (1)
    {
        if (xSemaphoreTake(g_Mutex, (TickType_t) MUTEX_WAIT_TIME) == pdTRUE)
        {
            char *task_stats_1 = (char *) malloc(1024); // Allocate buffer for task stats
            /* Se
            t GPIO2 low to turn LED off */
            gpio_set_level(GPIO_NUM_2, Logic_Low);
            vTaskGetRunTimeStats(task_stats_1); // Get task run time stats
            printf("Task Runtime Stats:\n%s\n", task_stats_1); // Print task stats
            xSemaphoreGive(g_Mutex);
...
void Task3(void *pv_parameters)
{
    ...
    while (1)
    {
        char *task_stats_2 = (char *) malloc(1024); // Allocate buffer for task stats

        /* Print status message via UART */
        printf("Task 3: Status message\n");
        vTaskGetRunTimeStats(task_stats_2); // Get task run time stats

        printf("Task Runtime Stats:\n%s\n", task_stats_2); // Print task stats
